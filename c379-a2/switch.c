                        ...
                        int IPlow, int IPhigh)
{
    ...
    init_switch(&sig_struct, trafficfile, traffic_stream, &keyboard);
...
void init_switch(struct sigaction *sig_struct, const char *trafficfile,
    ...
    snprintf(packet, MAX_PACKET_LENGTH,
        ...
        write_tofifo(fds_wr[0], packet);
        // wait for ACK
        char input[LINE_BUFFER];
        poll(&fds_rd[0], 1, -1); //hang forever waiting for ACK
        if (read_fromfifo(fds_rd[0].fd, input) == 0) {
            Packet rcv_pckt;
            str2pckt(input, &rcv_pckt);
